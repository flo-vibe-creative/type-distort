...
export function bulgeGeometry(params: BulgeParams, ctx: WarpContext): BulgeGeometry {
  const center = { x: params.cx * ctx.width, y: params.cy * ctx.height }
  const radius = Math.max(0, params.radius) * (Math.hypot(ctx.width, ctx.height) / 2)
  return {
    center,
    radius,
    peak: clampPeak(center, radius, params.peakX * ctx.width, params.peakY * ctx.height),
  }
}

/** 원 중심에서 (offsetX, offsetY)만큼 떨어진 중앙점을 원 안쪽으로 끌어들인다 */
export function clampPeak(center: Point, radius: number, offsetX: number, offsetY: number): Point {
...
export const warpBulge: WarpFn<BulgeParams> = (u, v, params, ctx) => {
  ...
  const reach =
    -along +
    Math.sqrt(
      Math.max(0, along * along - (offsetX * offsetX + offsetY * offsetY) + radius * radius)
    )
