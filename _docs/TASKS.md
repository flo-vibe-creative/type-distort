...
| `lib/warp/stack.ts` | 효과 목록 차례 적용·역계산 |
| `lib/warp/stackHandles.ts` | 펼친 효과의 조작점을 뒤 효과 거친 자리에 배치 |
...
- [x] 여러 레이어 함께 키우기·돌리기
- [x] 볼록/웨이브 중앙점을 끌면 점선 원은 제자리에 두고 가장 부푸는 지점만 원 안에서 이동 (원 전체는 점선 원을 잡아 끌기)
...
**Status:** ✅ 완료
**목표:** 한 레이어에 왜곡 효과를 2~3개 이상 겹쳐, 위에서부터 차례로 적용한 결과를 만들 수 있다.

- [x] `lib/warp/stack.ts` — 효과 목록을 차례로 통과시키는 계산, 뒤 효과를 거꾸로 풀어 조작점을 끄는 역계산
- [x] 레이어 문서 구조를 효과 목록(`warps`)으로 바꾸고, 예전 저장본(효과 1개)도 그대로 열리게
- [x] 화면·내보내기(벡터·이미지)·경계 계산이 효과 목록 전체를 반영
- [x] 펼친 효과의 조작점·안내선만 캔버스에 표시 — 뒤 효과를 거친 자리에 그리고, 끌면 역계산으로 반영
- [x] 패널: 효과 목록 + 펼치기, [+ 효과 추가], 끄기/켜기, 순서 ↑↓, 삭제, 같은 효과 중복 허용
- [x] 되돌리기·자동 저장 확인 후 커밋
